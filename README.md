...
**Mactoy does NOT auto-launch at login.** The pane is called "Login Items" but it has two distinct sections: *Open at Login* (apps that auto-launch) and *Allow in the Background* (daemons registered via `SMAppService`). Mactoy's toggle is in the second section — it allows the helper `mactoyd` to start on demand. **Mactoy.app itself never gets registered as a login item** (`grep SMLoginItem` and `grep "SMAppService.mainApp"` across the source tree both return zero hits — the only registration we ever issue is `SMAppService.daemon(plistName:)` for the helper). Mactoy.app stays closed until you open it manually.
